...
struct PreeeeeeeeeeessedDuck : Module {
    ...
    float fadeLevel[17] = {1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f,1.f};
    ...
    float sampleRate = 48000; //default 48000, update in config

    // UI CACHING: Only update knobs/buttons every N cycles
    const int UI_UPDATE_DIVIDER = 100;
    int uiUpdateCounter = 0;
    
    // Cached UI values
    float cachedVolume[16] = {1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 
                               1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f};
    float cachedPan[16] = {0.0f};
    float cachedSidechainVolume = 0.6f;
    float cachedDuck = 0.7f;
    float cachedDuckAtt = 0.0f;
    float cachedPress = 0.0f;
    float cachedPressAtt = 0.0f;
    float cachedFeedback = 0.0f;
    float cachedFeedbackAtt = 0.0f;
    float cachedMasterVol = 1.0f;
    float cachedMasterVolAtt = 0.0f;
    bool updateUI = true;
    ...
    PreeeeeeeeeeessedDuck() {
        ...
        configInput(AUDIO_7L_INPUT, "Chan. 7 L");
        configInput(AUDIO_7R_INPUT, "Chan. 7 R");
        configInput(AUDIO_8L_INPUT, "Chan. 8 L");
        configInput(AUDIO_8R_INPUT, "Chan. 8 R");
        configInput(AUDIO_9L_INPUT, "Chan. 9 L");
        configInput(AUDIO_9R_INPUT, "Chan. 9 R");
        configInput(AUDIO_10L_INPUT, "Chan. 10 L");
        configInput(AUDIO_10R_INPUT, "Chan. 10 R");
        configInput(AUDIO_11L_INPUT, "Chan. 11 L");
        configInput(AUDIO_11R_INPUT, "Chan. 11 R");
        configInput(AUDIO_12L_INPUT, "Chan. 12 L");
        configInput(AUDIO_12R_INPUT, "Chan. 12 R");
        configInput(AUDIO_13L_INPUT, "Chan. 13 L");
        configInput(AUDIO_13R_INPUT, "Chan. 13 R");
        configInput(AUDIO_14L_INPUT, "Chan. 14 L");
        configInput(AUDIO_14R_INPUT, "Chan. 14 R");
        configInput(AUDIO_15L_INPUT, "Chan. 15 L");
        configInput(AUDIO_15R_INPUT, "Chan. 15 R");
        configInput(AUDIO_16L_INPUT, "Chan. 16 L");
        configInput(AUDIO_16R_INPUT, "Chan. 16 R");
        ...
        configInput(VCA_CV7_INPUT, "Chan. 7 VCA CV");
        configInput(VCA_CV8_INPUT, "Chan. 8 VCA CV");
        configInput(VCA_CV9_INPUT, "Chan. 9 VCA CV");
        configInput(VCA_CV10_INPUT, "Chan. 10 VCA CV");
        configInput(VCA_CV11_INPUT, "Chan. 11 VCA CV");
        configInput(VCA_CV12_INPUT, "Chan. 12 VCA CV");
        configInput(VCA_CV13_INPUT, "Chan. 13 VCA CV");
        configInput(VCA_CV14_INPUT, "Chan. 14 VCA CV");
        configInput(VCA_CV15_INPUT, "Chan. 15 VCA CV");
        configInput(VCA_CV16_INPUT, "Chan. 16 VCA CV");
        configInput(VCA_SIDECHAIN_INPUT, "Sidechain VCA CV");


#ifdef METAMODULE    
        configInput(PAN_CV1_INPUT, "Chan. 1 Pan CV");
#else
        configInput(PAN_CV1_INPUT, "Chan. 1 Pan CV / Poly");
#endif
        configInput(PAN_CV2_INPUT, "Chan. 2 Pan CV");
        configInput(PAN_CV3_INPUT, "Chan. 3 Pan CV");
        configInput(PAN_CV4_INPUT, "Chan. 4 Pan CV");
        configInput(PAN_CV5_INPUT, "Chan. 5 Pan CV");
        configInput(PAN_CV6_INPUT, "Chan. 6 Pan CV");
        configInput(PAN_CV7_INPUT, "Chan. 7 Pan CV");
        configInput(PAN_CV8_INPUT, "Chan. 8 Pan CV");
        configInput(PAN_CV9_INPUT, "Chan. 9 Pan CV");
        configInput(PAN_CV10_INPUT, "Chan. 10 Pan CV");
        configInput(PAN_CV11_INPUT, "Chan. 11 Pan CV");
        configInput(PAN_CV12_INPUT, "Chan. 12 Pan CV");
        configInput(PAN_CV13_INPUT, "Chan. 13 Pan CV");
        configInput(PAN_CV14_INPUT, "Chan. 14 Pan CV");
        configInput(PAN_CV15_INPUT, "Chan. 15 Pan CV");
        configInput(PAN_CV16_INPUT, "Chan. 16 Pan CV");

#ifdef METAMODULE    
        configInput(MUTE_1_INPUT, "Chan. 1 Mute CV");
#else
        configInput(MUTE_1_INPUT, "Chan. 1 Mute CV / Poly");
#endif
        configInput(MUTE_2_INPUT, "Chan. 2 Mute CV");
        configInput(MUTE_3_INPUT, "Chan. 3 Mute CV");
        configInput(MUTE_4_INPUT, "Chan. 4 Mute CV");
        configInput(MUTE_5_INPUT, "Chan. 5 Mute CV");
        configInput(MUTE_6_INPUT, "Chan. 6 Mute CV");
        configInput(MUTE_7_INPUT, "Chan. 7 Mute CV");
        configInput(MUTE_8_INPUT, "Chan. 8 Mute CV");
        configInput(MUTE_9_INPUT, "Chan. 9 Mute CV");
        configInput(MUTE_10_INPUT, "Chan. 10 Mute CV");
        configInput(MUTE_11_INPUT, "Chan. 11 Mute CV");
        configInput(MUTE_12_INPUT, "Chan. 12 Mute CV");
        configInput(MUTE_13_INPUT, "Chan. 13 Mute CV");
        configInput(MUTE_14_INPUT, "Chan. 14 Mute CV");
        configInput(MUTE_15_INPUT, "Chan. 15 Mute CV");
        configInput(MUTE_16_INPUT, "Chan. 16 Mute CV");
    ...
    void process(const ProcessArgs& args) override {
		...
		updateUI = (uiUpdateCounter == 0);
		if (updateUI) {
			for (int i = 0; i < 16; i++) {
				cachedVolume[i] = params[VOLUME1_PARAM + i].getValue();
				cachedPan[i] = params[PAN1_PARAM + i].getValue();
			}
			cachedSidechainVolume = params[SIDECHAIN_VOLUME_PARAM].getValue();
			cachedDuck = params[DUCK_PARAM].getValue();
			cachedDuckAtt = params[DUCK_ATT].getValue();
			cachedPress = params[PRESS_PARAM].getValue();
			cachedPressAtt = params[PRESS_ATT].getValue();
			cachedFeedback = params[FEEDBACK_PARAM].getValue();
		...
		bool channelNeedsProcessing[16] = {false};
		int firstConnectedChannel = -1;
		
		for (int i = 0; i < 16; i++) {
			bool hasL = inputs[AUDIO_1L_INPUT + 2 * i].isConnected();
			bool hasR = inputs[AUDIO_1R_INPUT + 2 * i].isConnected();
			bool hasVCA = inputs[VCA_CV1_INPUT + i].isConnected();
			bool hasPan = inputs[PAN_CV1_INPUT + i].isConnected();
			bool hasMute = inputs[MUTE_1_INPUT + i].isConnected();
		...
		// Scan ALL channels to build polyphony map, but only read inputs for channels that need it
		if (updateUI){
			for (int i = 0; i < 16; i++) {
				activeAudio[i] = -1;
				activeVcaChannel[i] = -1;
				activePanChannel[i] = -1;
				activeMuteChannel[i] = -1;
			}
			for (int i = 0; i < 16; i++) {
				...
				if (i > 0) {
					if (activeAudio[i] == -1 && activeAudio[i-1] != -1) {
						if (audioChannels[activeAudio[i-1]] > (i - activeAudio[i-1])) {
							activeAudio[i] = activeAudio[i-1];
						}
					}
					if (activeVcaChannel[i] == -1 && activeVcaChannel[i-1] != -1) {
						if (vcaChannels[activeVcaChannel[i-1]] > (i - activeVcaChannel[i-1])) {
							activeVcaChannel[i] = activeVcaChannel[i-1];
						}
					}
					if (activePanChannel[i] == -1 && activePanChannel[i-1] != -1) {
						if (panChannels[activePanChannel[i-1]] > (i - activePanChannel[i-1])) {
							activePanChannel[i] = activePanChannel[i-1];
						}
					}
					if (activeMuteChannel[i] == -1 && activeMuteChannel[i-1] != -1) {
		...
		// Process channels - but only those that actually need processing or have audio
		for (int i = 0; i < 16; i++) {
		
			// ALWAYS process mute buttons for UI responsiveness (cheap operation)
        ...
        if (inputCount>0 && compressionAmountL > 0 && compressionAmountR > 0){  //div zero protection
            pressTotalL = ( (1.0f-pressAmount) + (pressAmount / compressionAmountL) ) * 16.0f / inputCount;
            pressTotalR = ( (1.0f-pressAmount) + (pressAmount / compressionAmountR) ) * 16.0f / inputCount;
        } else {
            pressTotalL = 0.0f;
            pressTotalR = 0.0f;
        }
        ...
        if (!isSideConnectedL && !isSideConnectedR) {
            ...
            // Measure envelope from pre-mix-scaled signal for clean linear ducking
            ...
            // Scale sidechain up to match main mix level (post envelope measurement)
            sideL *= 16.0f;
            sideR *= 16.0f;

            // Calculate ducking: full duck at 1.5V envelope so any reasonable signal ducks cleanly
        ...
        // -0.7 dB for the corrected form. Node.cpp already does it this way.
        float adaaInL = mixL / 85.f; //85 is 17x5v
        float adaaInR = mixR / 85.f;
...
struct PreeeeeeeeeeessedDuckWidget : ModuleWidget {
    ...
    struct FloatMemberQuantity : Quantity {
        PreeeeeeeeeeessedDuck* module;
        float PreeeeeeeeeeessedDuck::*member; // pointer-to-member
        std::string label;
        float min, max, def;
        int precision;
    
        FloatMemberQuantity(PreeeeeeeeeeessedDuck* m, float PreeeeeeeeeeessedDuck::*mem, std::string lbl,
    ...
    void appendContextMenu(Menu* menu) override {
        ...
        if (!PreeeeeeeeeeessedDuckModule) return;
        ...
        fadeSlider->quantity = new FloatMemberQuantity(PreeeeeeeeeeessedDuckModule, &PreeeeeeeeeeessedDuck::transitionTime,
